...
sleep 0.1 && mount_rgw $USERNAME || echo "[FAIL] Failed at mounting $USERNAME from rgw"
sleep 0.2 && mount_rgw scratch || echo "[FAIL] Failed at mounting scratch from rgw"

for additional_volume in $@;
do
  mount_rgw $additional_volume || echo "[FAIL] Failed at mounting $additional_volume from rgw"  ;
done
...
## Custom user setup
## -----------------
## Run the .cloud-profile script the user might have in his/her home directory.
################################################################################

if [ -f "/mlinfn/private/.cloud-profile" ]
then
  /bin/bash /home/$LOGNAME/.cloud-profile || echo "[FAIL] Error in .cloud-profile" &
fi
