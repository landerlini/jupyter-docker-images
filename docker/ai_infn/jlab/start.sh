...
if [ "$(id -u)" == 0 ]; then
    ...
    if id jovyan &> /dev/null; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" jovyan 2>&1 | grep "no changes" > /dev/null; then
            ...
            _log "- home dir: /home/jovyan -> ${NB_HOME}"
        fi
    ...
    if [ "${NB_UID}" != "$(id -u "${NB_USER}")" ] || [ "${NB_GID}" != "$(id -g "${NB_USER}")" ]; then
        ...
        userdel "${NB_USER}"
        useradd --no-log-init --home "${NB_HOME}" --shell /bin/bash --uid "${NB_UID}" --gid "${NB_GID}" --groups 100 "${NB_USER}"
    fi
    ...
    if [ "${NB_USER}" = "root" ] && [ "${NB_UID}" = "$(id -u "${NB_USER}")" ] && [ "${NB_GID}" = "$(id -g "${NB_USER}")" ]; then
        sed -i "s|/root|${NB_HOME}|g" /etc/passwd
    ...
    if [[ "${NB_USER}" != "jovyan" ]]; then
        if [[ ! -e "${NB_HOME}" ]]; then
            _log "Attempting to copy /home/jovyan to ${NB_HOME}..."
            mkdir "${NB_HOME}"
            # shellcheck disable=SC2086
            if cp ${CP_OPTS:--a} /home/jovyan/. "${NB_HOME}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/jovyan to ${NB_HOME}!"
                _log "Attempting to symlink /home/jovyan to ${NB_HOME}..."
                if ln -s /home/jovyan "${NB_HOME}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/jovyan to ${NB_HOME} or to create symlink!"
                    exit 1
                fi
            fi
        fi
    ...
    if [[ "${CHOWN_HOME}" == "1" || "${CHOWN_HOME}" == "yes" ]]; then
        _log "Ensuring ${NB_HOME} is owned by ${NB_UID}:${NB_GID} ${CHOWN_HOME_OPTS:+(chown options: ${CHOWN_HOME_OPTS})}"
        # shellcheck disable=SC2086
        chown ${CHOWN_HOME_OPTS} "${NB_UID}:${NB_GID}" "${NB_HOME}"
    fi
...
# user we started as.
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            ...
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:${NB_HOME}:/bin/bash" >> /tmp/passwd
